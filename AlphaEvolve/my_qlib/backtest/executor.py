...
from my_qlib.backtest.account import Account
    ...
    def _collect_data(
        ...
        while not self.inner_executor.finished():
            ...
            if not self._align_range_limit or start_idx <= sub_cal.get_trade_step() <= end_idx:
                ...
                # - Normally, user will create a strategy and embed it into my_qlib's executor and simulator interaction
                #   loop For a _nested my_qlib example_, (my_qlib Strategy) <=> (my_qlib Executor[(inner my_qlib Strategy) <=>
                #   (inner my_qlib Executor)])
                # - However, RL-based framework has it's own script to run the loop
                #   For an _RL learning example_, (RL Policy) <=> (RL Env[(inner my_qlib Executor)])
                # To make it possible to run  _nested my_qlib example_ and _RL learning example_ together, the solution
                # below is proposed
                # - The entry script follow the example of  _RL learning example_ to be compatible with all kinds of
                #   RL Framework
                # - Each step of (RL Env) will make (inner my_qlib Executor) one step forward
                #     - (inner my_qlib Strategy) is a proxy strategy, it will give the program control right to (RL Env)
