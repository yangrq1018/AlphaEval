...
from my_qlib.utils import exists_qlib_data
...
class GetData:
    ...
    def download_data(self, file_name: str, target_dir: [Path, str], delete_old: bool = True):
        ...
        ---------
        # get rl data
        python get_data.py download_data --file_name rl_data.zip --target_dir ~/.my_qlib/qlib_data/rl_data
        ...
        # get cn csv data
        python get_data.py download_data --file_name csv_data_cn.zip --target_dir ~/.my_qlib/csv_data/cn_data
    ...
    def _unzip(file_path: [Path, str], target_dir: [Path, str], delete_old: bool = True):
        ...
        if delete_old:
            logger.warning(
                f"will delete the old my_qlib data directory(features, instruments, calendars, features_cache, dataset_cache): {target_dir}"
            )
    ...
    def qlib_data(
        self,
        name="qlib_data",
        target_dir="~/.my_qlib/qlib_data/cn_data",
        version=None,
        interval="1d",
        region="cn",
        delete_old=True,
        ...
        """download cn my_qlib data from remote
        ...
        ---------
        # get 1d data
        python get_data.py qlib_data --name qlib_data --target_dir ~/.my_qlib/qlib_data/cn_data --interval 1d --region cn
        ...
        # get 1min data
        python get_data.py qlib_data --name qlib_data --target_dir ~/.my_qlib/qlib_data/cn_data_1min --interval 1min --region cn
        ...
        qlib_version = ".".join(re.findall(r"(\d+)\.+", my_qlib.__version__))
