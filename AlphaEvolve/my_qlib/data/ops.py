...
try:
    ...
    print(
        "#### Do not import my_qlib package in the repository directory in case of importing my_qlib from . without compiling #####"
    )
    raise
except ValueError:
...
# 全局行业映射变量（instrument -> industry）及其反向映射（industry -> instruments列表）
RELATION_MAP = None
RELATION_INV_MAP = None

def load_relation_map_from_csv(path):
    """从CSV文件加载 instrument -> industry 的映射。CSV需包含 'instrument' 和 'industry' 两列。"""
    import pandas as pd
    global RELATION_MAP, RELATION_INV_MAP
    df = pd.read_csv(path)
    # 将CSV的两列转换为字典（instrument为键，industry为值）
    RELATION_MAP = pd.Series(df['industry'].values, index=df['instrument']).to_dict()
    # 构建反向映射字典
    RELATION_INV_MAP = {}
    RELATION_INV_MAP["000000.SI"] = []
    for inst, ind in RELATION_MAP.items():
        RELATION_INV_MAP.setdefault(ind, []).append(inst)

def load_relation_map_from_dict(map_dict):
    """从已存在的字典加载 instrument -> industry 的映射。"""
    global RELATION_MAP, RELATION_INV_MAP
    RELATION_MAP = map_dict.copy()
    RELATION_INV_MAP = {}
    for inst, ind in RELATION_MAP.items():
        RELATION_INV_MAP.setdefault(ind, []).append(inst)


#################### Self-Designed Operator ####################
class RelationRank(ExpressionOps):
    """按照行业对特征进行横截面百分位排名的算子"""
    ...
    def _load_internal(self, instrument, start_index, end_index, *args):
        # 检查全局行业映射是否已加载且包含当前instrument
        if RELATION_MAP is None or instrument not in RELATION_MAP:
            print(f"Instrument {instrument} 的行业映射不存在，请先加载关系映射。")
        ...
        # 找到该股票所属的行业及同业股票列表
            ...
            industry = RELATION_MAP[instrument]
        group_instruments = RELATION_INV_MAP.get(industry, [])
        # 收集同一行业内每个股票在指定时间范围[start_index, end_index]的特征序列
        data_dict = {}
        for inst in group_instruments:
            series = self.feature.load(inst, start_index, end_index, *args)
            data_dict[inst] = series
        # 构造DataFrame使得index为日期，每列对应一个股票的特征值
        df = pd.DataFrame(data_dict)
        # 按行（日期）计算百分位排名
        rank_df = df.rank(pct=True, axis=1)
        # 提取当前 instrument 的排名序列，并仅保留该股票自身日期索引的数据
        base_series = data_dict[instrument]
        result = rank_df[instrument].reindex(base_series.index)
        return result


class RelationDemean(ExpressionOps):
    """按照行业对特征进行去均值（减去行业平均值）的算子"""
    def __init__(self, feature):
        self.feature = feature
    ...
    def _load_internal(self, instrument, start_index, end_index, *args):
        ...
        base_series = data_dict[instrument]
        # 计算当前股票特征减去当日行业均值的结果序列
        result = base_series.reindex(mean_series.index) - mean_series
        result = result.reindex(base_series.index)
        return result
    ...
    def register(self, ops_list: List[Union[Type[ExpressionOps], dict]]):
        ...
        for _operator in ops_list:
            ...
            if _ops_class.__name__ in self._ops:
                get_module_logger(self.__class__.__name__).warning(
                    "The custom operator [{}] will override the my_qlib default definition".format(_ops_class.__name__)
                )
...
def register_all_ops(C):
    ...
    from .pit import P, PRef  # pylint: disable=C0415
