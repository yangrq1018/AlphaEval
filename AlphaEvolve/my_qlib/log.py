...
class _QLibLoggerManager:
    ...
    def __call__(self, module_name, level: Optional[int] = None) -> QlibLogger:
        ...
        if not module_name.startswith("my_qlib."):
            # Add a prefix of my_qlib. when the requested ``module_name`` doesn't start with ``my_qlib.``.
            # If the module_name is already my_qlib.xxx, we do not format here. Otherwise, it will become my_qlib.my_qlib.xxx.
            module_name = "my_qlib.{}".format(module_name)
...
def set_global_logger_level(level: int, return_orig_handler_level: bool = False):
    """set my_qlib.xxx logger handlers level
    ...
    ---------
        ...
        .. code-block:: python
            ...
            import my_qlib
            import logging
            from my_qlib.log import get_module_logger, set_global_logger_level
            my_qlib.init()
    ...
    """
    _handler_level_map = {}
    qlib_logger = logging.root.manager.loggerDict.get("my_qlib", None)  # pylint: disable=E1101
...
def set_global_logger_level_cm(level: int):
    """set my_qlib.xxx logger handlers level to use contextmanager
    ...
    ---------
        ...
        .. code-block:: python
            ...
            import my_qlib
            import logging
            from my_qlib.log import get_module_logger, set_global_logger_level_cm
            my_qlib.init()
