...
from my_qlib.model.trainer import task_train
from my_qlib.utils import set_log_with_config
...
def workflow(config_path, experiment_name="workflow", uri_folder="mlruns"):
    """
    This is a my_qlib CLI entrance.
    User can run the whole Quant research workflow defined by a configure file
    - the code is located here ``my_qlib/workflow/cli.py`

    User can specify a base_config file in your workflow.yml file by adding "BASE_CONFIG_PATH".
    my_qlib will load the configuration in BASE_CONFIG_PATH first, and the user only needs to update the custom fields
    ...
    if "exp_manager" in config.get("qlib_init"):
        my_qlib.init(**config.get("qlib_init"))
    else:
        exp_manager = C["exp_manager"]
        exp_manager["kwargs"]["uri"] = "file:" + str(Path(os.getcwd()).resolve() / uri_folder)
        my_qlib.init(**config.get("qlib_init"), exp_manager=exp_manager)
