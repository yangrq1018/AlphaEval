...
from my_qlib.utils import hash_args
from my_qlib.utils.mod import init_instance_by_config
...
def replace_task_handler_with_cache(task: dict, cache_dir: Union[str, Path] = ".") -> dict:
    ...
    >>> task = {"dataset": {"kwargs":{'handler': {'class': 'Alpha158', 'module_path': 'my_qlib.contrib.data.handler', 'kwargs': {'start_time': datetime.date(2008, 1, 1), 'end_time': datetime.date(2020, 8, 1), 'fit_start_time': datetime.date(2008, 1, 1), 'fit_end_time': datetime.date(2014, 12, 31), 'instruments': 'CSI300'}}}}}
