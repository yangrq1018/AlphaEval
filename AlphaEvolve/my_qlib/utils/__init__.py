...
def get_date_by_shift(
    ...
    """
    from my_qlib.data import D  # pylint: disable=C0415
...
def time_to_slc_point(t: Union[None, str, pd.Timestamp]) -> Union[None, pd.Timestamp]:
    """
    Time slicing in my_qlib or Pandas is a frequently-used action.
    ...
    """
    if t is None:
        # None represents unbounded in my_qlib or Pandas(e.g. df.loc[slice(None, "20210303")]).
        return t
    else:
...
def check_qlib_data(qlib_config):
    ...
    for _p in inst_dir.glob("*.txt"):
        assert len(pd.read_csv(_p, sep="\t", nrows=0, header=None).columns) == 3, (
            f"\nThe {str(_p.resolve())} of my_qlib data is not equal to 3 columns:"
            f"\n\tIf you are using the data provided by my_qlib: "
            f"https://my_qlib.readthedocs.io/en/latest/component/data.html#my_qlib-format-dataset"
            f"\n\tIf you are using your own data, please dump the data again: "
            f"https://my_qlib.readthedocs.io/en/latest/component/data.html#converting-csv-format-into-my_qlib-format"
        )
    ...
    config : dict
        ...
        {'dataset': {'class': 'DatasetH',
          'kwargs': {'handler': {'class': 'Alpha158',
                                 ...
                                 'module_path': 'my_qlib.contrib.data.handler'},
...
#################### Wrapper #####################
class Wrapper:
    """Wrapper class for anything that needs to set up during my_qlib.init"""
    ...
    def __getattr__(self, key):
        if self.__dict__.get("_provider", None) is None:
            raise AttributeError("Please run my_qlib.init() first using my_qlib")
