...
class QRQCMAgent(BaseAgent):

    def __init__(self, env, valid_calculator, test_calculator, log_dir,
                 num_steps=5*(10**7), std_lam = 1, skew_lam = 0, kurt_lam = 0, 
                 ...
                 seed=0):
        super(QRQCMAgent, self).__init__(
        ...
        self.online_net = QRDQN(num_actions=self.num_actions,
                                N=N,
                                dueling_net=dueling_net,
                                noisy_net=noisy_net,
                                require_QCM=True).to(self.device)
        self.target_net = QRDQN(num_actions=self.num_actions,
                                N=N,
                                dueling_net=dueling_net,
                                noisy_net=noisy_net,
                                require_QCM=True).to(self.device)
        ...
        # Fixed fractions.
        taus = torch.arange(0, N+1, device=self.device, dtype=torch.float32) / N
    ...
    def learn(self):
        ...
        quantile_loss, mean_q, errors = self.calculate_loss(states, actions, rewards, next_states, dones, weights)
