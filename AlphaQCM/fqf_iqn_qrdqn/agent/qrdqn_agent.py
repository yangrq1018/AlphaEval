...
class QRDQNAgent(BaseAgent):
    ...
    def __init__(self, env, valid_calculator, test_calculator, log_dir, num_steps=5*(10**7),
        ...
        self.online_net = QRDQN(num_actions=self.num_actions, N=N,dueling_net=dueling_net,
            noisy_net=noisy_net).to(self.device)
        # Target network.
        ...
        self.target_net = QRDQN(num_actions=self.num_actions, N=N,dueling_net=dueling_net,
            noisy_net=noisy_net).to(self.device)
    ...
    def learn(self):
        ...
        if self.steps % self.log_interval == 0:
            self.writer.add_scalar(
                'loss/quantile_loss', quantile_loss.detach().item(), self.steps)
