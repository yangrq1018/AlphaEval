...
from my_qlib.data.data import LocalDatasetProvider
...
class AlphaEval:
    def __init__(
        ...
        if weights:
            ...
            self.weights = WeightCalculator(self.factor_expressions, train_start_date, train_end_date, instruments).train_optimal_weights()
        ...
        my_qlib.init(provider_uri="path/to/your/qlib_data", region="cn")
    ...
    def calculate_covariance_entropy(self) -> None:
        ...
        mat = clean_df.values  # shape = (n_samples, m)
        if mat.shape[0] < 2:
            raise ValueError(
                "丢弃 NaN 后样本数不足，无法计算协方差（需要至少 2 个样本）。"
            )
    ...
    def LLM_scores(
        ...
        client = OpenAI(
            api_key="Your own LLM key. If it is not gpt series, url is necessarily added.",
        )
