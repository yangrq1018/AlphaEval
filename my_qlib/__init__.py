...
def init(default_conf="client", **kwargs):
    ...
    **kwargs :
    ...
    if skip_if_reg and C.registered:
        # if we reinitialize my_qlib during running an experiment `R.start`.
    ...
    logger.info("my_qlib successfully initialized based on %s settings." % default_conf)
...
def init_from_yaml_conf(conf_path, **kwargs):
    ...
    :param conf_path: A path to the my_qlib config in yml format
    """
...
def get_project_path(config_name="config.yaml", cur_path: Union[Path, str, None] = None) -> Path:
    ...
    - my_qlib is a sub folder in project path
    - There is a file named `config.yaml` in my_qlib.
    ...
    - user want to use a relative config path instead of hard-coding my_qlib config path in code
...
def auto_init(**kwargs):
    """
    This function will init my_qlib automatically with following priority
    - Find the project configuration and init my_qlib
        - The parsing process will be affected by the `conf_type` of the configuration file
    - Init my_qlib with default config
    ...
    try:
        ...
        if conf_type == "origin":
            # The type of config is just like original my_qlib config
