...
from my_qlib.utils.time import epsilon_change

if TYPE_CHECKING:
    from my_qlib.backtest.decision import BaseTradeDecision
...
class TradeCalendarManager:
    ...
    def get_step_time(self, trade_step: int | None = None, shift: int = 0) -> Tuple[pd.Timestamp, pd.Timestamp]:
        ...
        About the endpoints:
            - my_qlib uses the closed interval in time-series data selection, which has the same performance as
            pandas.Series.loc
            # - The returned right endpoints should minus 1 seconds because of the closed interval representation in
            #   my_qlib.
            # Note: my_qlib supports up to minutely decision execution, so 1 seconds is less than any trading time
            #   interval.
