...
class RelationRank(ExpressionOps):
    ...
    def _load_internal(self, instrument, start_index, end_index, *args):
        if RELATION_MAP is None or instrument not in RELATION_MAP:
            base = self.feature.load(instrument, start_index, end_index, *args)
            return pd.Series(np.random.rand(len(base)), index=base.index, name=instrument)

        industry = RELATION_MAP[instrument]
        key = (industry, start_index, end_index)

        if key not in self._cache:
            members = RELATION_INV_MAP.get(industry, [])
            data = {
                inst: self.feature.load(inst, start_index, end_index, *args)
                for inst in members
            }
            df = pd.DataFrame(data)
            rank_df = df.rank(pct=True, axis=1)
            self._cache[key] = rank_df

        rank_df = self._cache[key]
        base = self.feature.load(instrument, start_index, end_index, *args)
        return rank_df[instrument].reindex(base.index)
...
class RelationDemean(ExpressionOps):
    ...
    def _load_internal(self, instrument, start_index, end_index, *args):
        if RELATION_MAP is None or instrument not in RELATION_MAP:
            base = self.feature.load(instrument, start_index, end_index, *args)
            return pd.Series(0.0, index=base.index, name=instrument)
        ...
        if key not in self._cache:
            ...
            df = pd.DataFrame(data)
            mean_ser = df.mean(axis=1, skipna=True)
            self._cache[key] = mean_ser

        mean_ser = self._cache[key]
        base = self.feature.load(instrument, start_index, end_index, *args)
        demeaned = base.reindex(mean_ser.index) - mean_ser
        return demeaned.reindex(base.index)
