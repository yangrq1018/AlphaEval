...
class ProviderBackendMixin:
    ...
    def get_default_backend(self):
        ...
        backend.setdefault("module_path", "my_qlib.data.storage.file_storage")
        return backend
...
class PITProvider(abc.ABC):
    ...
    def period_feature(
        ...
        period: int
            ...
            The period is represented with int in my_qlib. (e.g. 202001 may represent the first quarter in 2020)
...
class DatasetProvider(abc.ABC):
    ...
    def _uri(
        ...
        **kwargs,
        ...
        """
        # TODO: my_qlib-server support inst_processors
    ...
    def load_calendar(self, freq, future):
        ...
        try:
            ...
            if future:
                ...
                get_module_logger("data").warning(
                    "You can get future calendar by referring to the following document: https://github.com/microsoft/my_qlib/blob/main/scripts/data_collector/contrib/README.md"
                )
...
class ClientDatasetProvider(DatasetProvider):
    ...
    def dataset(
        ...
        if disk_cache == 0:
            ...
            """
            # TODO: support inst_processors, need to change the code of my_qlib-server at the same time
            ...
            if inst_processors:
                raise ValueError(
                    ...
                    f"Please use `D.features(disk_cache=0)` or `my_qlib.init(dataset_cache=None)`"
                )
            self.conn.send_request(
                request_type="feature",
                request_content={
...
class BaseProvider:
    ...
    To keep compatible with old my_qlib provider.
    """
...
def register_all_wrappers(C):
    ...
    module = get_module_by_module_path("my_qlib.data")
    ...
    register_wrapper(Cal, _calendar_provider, "my_qlib.data")
    ...
    register_wrapper(Inst, _instrument_provider, "my_qlib.data")
    ...
    if getattr(C, "feature_provider", None) is not None:
        ...
        register_wrapper(FeatureD, feature_provider, "my_qlib.data")
    ...
    if getattr(C, "pit_provider", None) is not None:
        ...
        register_wrapper(PITD, pit_provider, "my_qlib.data")
    ...
    if getattr(C, "expression_provider", None) is not None:
        ...
        register_wrapper(ExpressionD, _eprovider, "my_qlib.data")
    ...
    register_wrapper(DatasetD, _dprovider, "my_qlib.data")
    logger.debug(f"registering DatasetD {C.dataset_provider}-{C.dataset_cache}")

    register_wrapper(D, C.provider, "my_qlib.data")
