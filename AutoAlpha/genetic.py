...
from .config import functions_arity
...
def _parallel_evolve(n_programs, parents, X_shape, y, sample_weight, seeds, params):
    ...
    for i in range(n_programs):
        ...
        if parents is None:
            ...
            print(str(donor), donor.depth_)
            print(str(parent), parent.depth_)
        ...
        son1 = _Program(function_set=function_set,
                           arities=arities,
        ...
        son2 = _Program(function_set=function_set,
                           arities=arities,
                           init_depth=init_depth,
                           init_method=init_method,
                           n_features=n_features,
        ...
        print("parents: ", str(parent), str(donor), "\n", "sons: ", str(son1), str(son2))
        son1.parents = genome
        son2.parents = genome
        ...
        indices, not_indices = son1.get_all_indices(n_samples,
                                                       max_samples,
                                                       random_state)
        curr_sample_weight[not_indices] = 0
        oob_sample_weight[indices] = 0

        son1.raw_fitness_ = son1.raw_fitness(X_shape, y, curr_sample_weight)
        if max_samples < n_samples:
            # Calculate OOB fitness
            son1.oob_fitness_ = son1.raw_fitness(X_shape, y, oob_sample_weight)

        indices, not_indices = son2.get_all_indices(n_samples,
                                                       max_samples,
                                                       random_state)
        curr_sample_weight[not_indices] = 0
        oob_sample_weight[indices] = 0

        son2.raw_fitness_ = son2.raw_fitness(X_shape, y, curr_sample_weight)
        if max_samples < n_samples:
            # Calculate OOB fitness
            son2.oob_fitness_ = son2.raw_fitness(X_shape, y, oob_sample_weight)
...
def _parallel_growth(n_programs, k, parents, X_shape, y, sample_weight, seeds, params):
    """Private function used to build a batch of programs within a job."""
    n_samples, n_features = X_shape
    # Unpack parameters
    tournament_size = params['tournament_size']
    ...
    for i in range(k * n_programs):
        ...
        if parents is None:
            ...
            partner_index = random.randint(0, len(parents) - 1)
            partner = parents[partner_index]

            program = parent.depth_evolution(partner, random_state)
            genome = {'method': 'Crossover',
                        'parent_idx': parent_index,
                        'parent_nodes': parent,
                        'partner_idx': partner_index,
        ...
        program.raw_fitness_ = program.raw_fitness(X_shape, y, curr_sample_weight)
        if max_samples < n_samples:
            # Calculate OOB fitness
            program.oob_fitness_ = program.raw_fitness(X_shape, y, oob_sample_weight)
...
class BaseSymbolic(BaseEstimator, metaclass=ABCMeta):
    ...
    def __init__(self,
        ...
        self.y = data.squeeze().to_numpy()

        self.X_shape = (len(self.y), len(self.feature_names))
    ...
    def fit(self, sample_weight=None):
        ...
        for function in self.function_set:
            if isinstance(function, str) and function in functions_arity:
        ...
        for function in self._function_set:
            arity = functions_arity[function]
        ...
        params['qlib_config'] = self.qlib_config
        ...
        for _ in range(self.population_size):
            program = _Program(function_set=params['function_set'],
                            arities=params['arities'],
                            init_depth=[1, 1],
                            init_method=params['init_method'],
                            n_features=n_features,
        ...
        for gen in range(prior_generations, self.generations):
            print(f"Generation {gen} starts.")
            curr_depth = gen
            params['curr_depth'] = curr_depth

            start_time = time()

            if gen == 0:
                parents = warm_start_parents
            else:
            ...
            total_candidates = self.growth_k * self.population_size
            all_seeds_for_growth = random_state.randint(MAX_INT, size=total_candidates)

            candidates_chunks = Parallel(n_jobs=n_jobs,
                                        verbose=int(self.verbose > 1))(
                delayed(_parallel_growth)(
                    n_programs[i],               
                    self.growth_k,
                    parents,
                    X_shape, y,
                    sample_weight,
                    all_seeds_for_growth[
                        starts[i] * self.growth_k : starts[i + 1] * self.growth_k
                    ],
                    params
                )
                for i in range(n_jobs)
            )
            all_candidates = list(itertools.chain.from_iterable(candidates_chunks))

            if self._metric.greater_is_better:
                all_candidates.sort(key=lambda prog: prog.raw_fitness_, reverse=True)
            else:
                all_candidates.sort(key=lambda prog: prog.raw_fitness_)

            parents_for_evolve = all_candidates[: self.population_size]

            print(f"Warm Start Finished: Generation {gen}.")

            parsimony_coefficient = None
            if self.parsimony_coefficient == 'auto':
                parsimony_coefficient = (np.cov(length, fitness)[1, 0] /
                                         np.var(length))
            for program in parents_for_evolve:
            ...
            print(f"Replacement Finished: Generation {gen}.")
        ...
        if isinstance(self, TransformerMixin):
            ...
            evaluation = np.array([gp.execute(X_shape) for gp in
...
class SymbolicTransformer(BaseSymbolic, TransformerMixin):
    ...
    def transform1(self):
        ...
        X_new = np.array([gp.execute(self.X_shape) for gp in self._best_programs]).T

        return X_new

    def fit_transform1(self, sample_weight=None):
        ...
        """
        return self.fit(sample_weight).transform1()
