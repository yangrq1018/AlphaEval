...
def _parallel_evolve(n_programs, parents, X, y, sample_weight, seeds, params):
    ...
    for i in range(n_programs):
        ...
        son1.raw_fitness_ = son1.raw_fitness(X, y, curr_sample_weight)
        if max_samples < n_samples:
            # Calculate OOB fitness
            son1.oob_fitness_ = son1.raw_fitness(X, y, oob_sample_weight)
        ...
        son2.raw_fitness_ = son2.raw_fitness(X, y, curr_sample_weight)
        if max_samples < n_samples:
            # Calculate OOB fitness
            son2.oob_fitness_ = son2.raw_fitness(X, y, oob_sample_weight)
...
def _parallel_growth(n_programs, k, parents, X, y, sample_weight, seeds, params):
    """Private function used to build a batch of programs within a job."""
    n_samples, n_features = X.shape
    # Unpack parameters
    tournament_size = params['tournament_size']
...
class BaseSymbolic(BaseEstimator, metaclass=ABCMeta):
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        for gen in range(prior_generations, self.generations):
            ...
            # seeds 数量要 >= k * population_size，如果希望每个候选都用不同种子
            # 这里我们按「每代一串总共需要 k*population_size」来生成
            ...
            # 现在把 total_candidates 均分给各 worker
            # 比如：worker i 需要生成 k * n_programs[i] 个 candidate
            # 事实上 n_programs 是“每个并行子任务要产出的 population_size 段”，
            # 但如果直接用它，k*n_programs 就正好是「该子任务要产出多少候选」
            candidates_chunks = Parallel(n_jobs=n_jobs,
                                        verbose=int(self.verbose > 1))(
                delayed(_parallel_growth)(
                    n_programs[i],               # 原本要生成的 n_programs，现在扩成 k * n_programs
                    self.growth_k,
                    parents,
                    X, y,
                    sample_weight,
            ...
            # 现在 all_candidates 中包含 k*population_size 个 _Program 对象，
            # 他们都已经计算好了 raw_fitness_ 和 oob_fitness_。下一步我们要从中筛 top population_size 个。

            # 1) 先把 (fitness, program) 组成一个列表
            #    注意：raw_fitness 存在 program.raw_fitness_，真正比较时要看 metric.greater_is_better
