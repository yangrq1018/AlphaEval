...
class _Program(object):
    ...
    def get_subtree(self, random_state, program=None, target_depth=None):
        ...
        random_state : RandomState instance
            随机数生成器。
        program : list, optional (default=None)
            要操作的扁平化表达式树；None 则使用 self.program。
        target_depth : int, optional (default=None)
            如果指定了深度，则只在该深度 level 选交叉点；深度从 0（根）开始计。

        Returns
        -------
        start, end : tuple of two ints
            子树在扁平列表中的起始和结束索引 [start, end)。
        """
        if program is None:
    ...
    #     probs = np.array([0.9 if isinstance(node, str) and node in functions_arity else 0.1
    #                       for node in program])
    #     probs = np.cumsum(probs / probs.sum())
    #     start = np.searchsorted(probs, random_state.uniform())
    ...
    #     for node in mutate:
    #         if isinstance(program[node], str) and program[node] in functions_arity:
    #             arity = functions_arity[program[node]]
